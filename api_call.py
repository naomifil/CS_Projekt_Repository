...
# ---------------------------------------------------------------------------
# Internal helpers
# Generated by ChatGPT, except for _get_api_key
...
# Used in the first iteration of our API calls
...
# takes measurements of each location and avarages it so that we only have one output per location
...
# used to find suitable stations for locations, in our case used in one shot station query
def _list_stations(
    client: OpenAQ,
    coordinate: tuple[float, float],
    radius: int,
...
# function only used in first attempt one shot to determine relevant stations
def find_stations(
    coordinate: tuple[float, float],
    radius: int,
    limit: int,
...
# gets aggregated readings for a list of stations and parameters
...
# gets aggregated readings for a list parameters for stations stored in stations.json
...
# gets weather information for a list of locatins
